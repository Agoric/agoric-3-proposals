...
- 0fccfb6: fix(synthetic-chain): restore `@agoric/smart-wallet` types for `tsc`
...
- 7fc7970: feat(synthetic-chain): set `$PUBLISHED_RPC_ENDPOINT` for `test.sh`

  Allow `before-test-run.sh` and `after-test-run.sh` not to hardcode the chain's
  `<host>:<port>` RPC endpoint.

- da815de: Ensure compatibility with Node.js v24
